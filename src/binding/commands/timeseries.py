...
from binding.core import available_summary, find_frames, load_stack
...
def timeseries(
    ...
    try:
        ...
        resolved_center_y = (
            center_y if center_y is not None else reference_image.shape[0] // 2
        )
        resolved_center_x = (
            center_x if center_x is not None else reference_image.shape[1] // 2
        )
        ...
        for size in roi_sizes:
            ...
            if time_real_by_index is not None:
                if time_index not in time_real_by_index:
                    raise typer.BadParameter(
                        f"time_map missing entry for time={time_index}"
                    )
    ...
    typer.echo(
        f"Saved {output_path} with rows={len(rows)}, times={len(times)}, "
        f"roi_sizes={roi_sizes}, center=({resolved_center_y}, {resolved_center_x})"
    )
